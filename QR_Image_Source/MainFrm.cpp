...
{
	static BOOL bRestore = FALSE; // 最小化からの復元時は処理しない 从最小化恢复时将不会被处理
...
// 用  途：ＱＲコードイメージを作成   生成二维码
// 备  注：実際の描画はCImageView::OnDraw    实际的绘图CImageView::OnDraw
...
void CMainFrame::ShowImage()
{
	...
	strEncodeData = "http://www.baidu.com";
	int i, j;
...
//		   フォーマットはDIB形式を使用
//			OLE拖和下降如果你却是指定的，不能以使用DIB格式拷贝到剪贴板格式时指定
...
void CMainFrame::CopyImage(COleDataSource* pOleDataSource)
{
	...
	if (hGlobal == NULL)
	{
		//MessageBox (" 内存不足。", NULL, MB_ICONEXCLAMATION );
		// 这里注释是因为显示错误	2	error C2001: 常量中有换行符	e:\baiduyundownload\qr_image_source\qr_image_source - 副本\mainfrm.cpp	460	1	QR_Image
		//错误	3	error C2143: 语法错误 : 缺少“)”(在“return”的前面)	e:\baiduyundownload\qr_image_source\qr_image_source - 副本\mainfrm.cpp	461	1	QR_Image
...
void CMainFrame::SaveAsImage()
{
	...
	pFileDialog->m_ofn.lpstrTitle = "QR Code asdf?";
...
void CMainFrame::SaveImage()
{
	...
	{
		m_strSavePath.Empty();
		MessageBox("路径错误", "QR Code 不能写入?, MB_ICONSTOP");
		return;
	}
